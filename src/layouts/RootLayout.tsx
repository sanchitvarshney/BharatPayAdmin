...
const menuItems = [
  ...
  { title: "User", tab: "user", icon: Icons.user, navigateTo: "/user/add-user" },
  { title: "Role", tab: "role", icon: Icons.role, navigateTo: "/role/list" },
  { title: "Location", tab: "location", icon: Icons.location, navigateTo: "/location/list" },
  { title: "Menu", tab: "menu", icon: Icons.menu, navigateTo: "/menu/create" },
  { title: "Permission", tab: "permission", icon: Icons.permission, navigateTo: "/permission/list" },
];
const profileItem = {
  title: "Profile",
  tab: "profile",
      ...
      <Accordion type="single" className="w-full" collapsible>
        <ul className="flex flex-col gap-[10px]  p-[10]">
          {menu.map((item: any, index: number) =>
            item?.menu_key === r?.menu_key || item?.parent_menu_key === r?.menu_key ? (
              <li key={index}>
                {item?.children ? (
                  <AccordionItem
                  ...
                  >
                    ...
                    <AccordionContent className="p-[10px] mt-[10px] border-l-2 border-yellow-600 bg-white rounded">{renderMenu(item.children, r, setSidemenu)}</AccordionContent>
                  </AccordionItem>
                ) : (
                  <div className="flex items-center justify-between w-full">
                    <Link onClick={() => setSidemenu(false)} to={item.url} className="w-full hover:no-underline hover:bg-[#DBEAFE] p-[10px] rounded-md cursor-pointer flex items-center gap-[10px]">
                      {item.name}
      ...
      <main className="h-[calc(100vh-20px)] bg-white rounded-md overflow-hidden flex">
        <div className="sidebar min-w-[60px] h-full bg-white border-r py-[20px]">
          ...
          <div className="h-[calc(100vh-160px)] flex flex-col items-center gap-[5px] py-[20px]">
            {menuItems.map(({ title, tab: itemTab, icon: Icon, navigateTo }) => (
              <Tooltip key={itemTab} title={title} placement="right" arrow>
                ...
                >
                  <Icon className={tab === itemTab ? "text-cyan-700" : "text-slate-700"} />
                </Button>
              </Tooltip>
            ))}
          </div>
          <div className="flex flex-col gap-4 mt-auto">
            <div className="h-[20px] flex items-center justify-center">
              <Tooltip title={profileItem.title} placement="right" arrow>
                ...
                >
                  <profileItem.icon className={tab === profileItem.tab ? "text-cyan-700" : "text-slate-700"} />
                </Button>
              </Tooltip>
            </div>
          </div>
